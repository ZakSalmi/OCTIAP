...
async def second_pipeline(headers, indicators, kafka_bootstrap_servers, group_id, kafka_general_topic, kafka_second_topic, producer):
    ...
    await produce_to_kafka(top_threat_sources, kafka_bootstrap_servers, kafka_second_topic)
...
if __name__ == "__main__":
    ...
    try:
        result = asyncio.run(second_pipeline(HEADERS, ipv4_indicators, KAFKA_BOOTSTRAP_SERVER, GROUP_ID, KAFKA_GENERAL_TOPIC, KAFKA_THREAT_TOPIC, producer))
        pprint(result)
    except KeyboardInterrupt:
