...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    filename="logs/seventh_pipeline.log",
    filemode="w",
    level=logging.INFO,
)
...
async def seventh_pipeline(
    ...
    try:
        ...
        await produce_to_kafka(
            producer,
            dict(malware_families.most_common(10)),
            logger,
            kafka_second_topic,
        )

        report_task = asyncio.create_task(
            consume_from_kafka(malware_families_consumer, logger, kafka_second_topic)
        )
